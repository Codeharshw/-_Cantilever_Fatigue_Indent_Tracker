...
# --- CONFIGURATION ---
# =============================================================================
# ⬇ CHANGE THIS: Define your experimental sessions/days.
# Each tuple: ("image_filename_prefix", start_hour_of_session)
# The prefix must match the beginning of your image filenames exactly.
# Add or remove entries depending on how many sessions you recorded.
# Example for 2 days: [("Day1", 0), ("Day2", 24)]
# Example for 4 days: [("Day1", 0), ("Day2", 24), ("Day3", 48), ("Day4", 72)]
SEGMENT_CONFIG = [
...
# ⬇ CHANGE THIS: File extension of your images.
# Common options: ".png", ".jpg", ".tif", ".bmp"
FILE_EXTENSION = ".png"

# ⬇ CHANGE THIS: Folder where your images are stored.
# "." means the same folder as this script.
# For a specific folder use e.g.: r"C:\Users\YourName\experiment_images"
SEARCH_DIR = "."

# ⬇ CHANGE THIS (optional): Name of the saved template file.
# This file is auto-created the first time you run the script.
# Delete it to force re-selection of a new template on next run.
TEMPLATE_FILENAME = "indent_template.png"

# --- SETTINGS ---
# These can be left as-is for most experiments. See notes below if you need to tune.
ALIGN_METHOD = "ECC"     # Image alignment method — ECC is most accurate, keep as-is
REMOVE_ZEROS = True      # Filter out frames where intensity drops to zero (camera dropout)
STITCH_DATA = True       # Smooth intensity jumps at session boundaries — recommended ON
VERIFY_SAMPLES = 15      # Max number of verification snapshots saved — increase if you want more

# Global variables for mouse interaction — do not change these
selected_point = None
...
def normalize_brightness(img, ref_stats):
    ...
    This corrects for lighting changes between sessions — do not remove.
    """
    if len(img.shape) == 3:
...
def load_or_create_template(img_gray):
    """
    Loads template from file or asks user to select one if missing.
    The template is a small patch around ONE indent — used to find all others.
    """
...
def detect_grid_candidates(img_gray, template):
    """
    Uses Multi-Scale Template Matching to find all potential indents.
    
    ⬇ TUNE IF NEEDED:
    - threshold (line ~130): lower value (e.g. 0.25) finds more candidates,
      higher value (e.g. 0.5) is stricter. Adjust if too many/few indents detected.
    - min_dist (line ~142): minimum pixel distance between two detections.
      Increase if the same indent is being detected twice.
    """
    ...
    detected_centers = []
    min_dist = 30  # ⬅ TUNE: increase if same indent detected twice (e.g. 50), decrease if nearby indents missed (e.g. 15)
...
def get_tracking_template(img, x, y, size=40):
    """
    Extracts a small image patch around (x, y) to use as a tracking template.
    
    ⬇ TUNE IF NEEDED:
    - size=40: the patch size in pixels. If your indents are very large,
      increase to e.g. 60 or 80. If very small, decrease to 20.
    """
    if len(img.shape) == 3:
...
def track_template_local(img, start_x, start_y, tracking_template, search_window=80):
    """
    Searches for the tracking template within a window around the last known position.
    
    ⬇ TUNE IF NEEDED:
    - search_window=80: how far (pixels) to search from last position.
      Increase if your specimen moves a lot between frames (e.g. 150).
      Decrease if you get false matches from nearby features (e.g. 40).
    """
    if len(img.shape) == 3:
        gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
    else:
...
def select_indents_manual_roi(img, template, title_prefix="", pre_selected=None):
    """
    Opens an interactive window for the user to click and select indents.
    
    Controls:
      - LEFT CLICK near an indent to select it (snaps to exact center)
      - Press 'D' to delete the last selected indent
      - Press 'C' to confirm your selection and proceed
      - Press ESC to cancel
    
    ℹ NOTE ON NUMBER OF INDENTS:
    There is NO hard limit on how many indents you can select.
    - For fewer than 25 indents: just click fewer points, press C when done.
    - For more than 25 indents: just keep clicking, press C when done.
    - The plotting grid auto-scales to however many indents you select.
    """
    global last_click
    last_click = None
    ...
    search_radius = 30  # ⬅ TUNE: pixel radius around click to search for indent center
...
def get_time_from_filename(filename, start_hour):
    """
    Extracts time from filenames in the format: 'Prefix (N).png'
    where N is the frame number.
    
    ⬇ CHANGE THIS if your filenames use a different naming convention.
    For example, if your files are named 'frame_001.png', 'frame_002.png':
        match = re.search(r'frame_(\d+)', filename)
        if match:
            frame_number = int(match.group(1))
            return start_hour + (frame_number - 1) * 0.25   # adjust 0.25 to your time-per-frame in hours
    """
    match = re.search(r'\s\((\d+)\)', filename)
    if match:
        frame_number = int(match.group(1))
        return start_hour + (frame_number - 1) * 0.25  # ⬅ CHANGE 0.25 to your actual time interval between frames (in hours)
    return None

def align_image_ecc(target, reference):
    """
    Aligns 'target' image to 'reference' using Enhanced Correlation Coefficient.
    This corrects for small camera movements between frames.
    
    ⬇ OPTIONAL — Can be commented out if your camera is perfectly stable
    and images are already aligned. In that case, set aligned = img in the main loop.
    
    ⬇ TUNE IF NEEDED:
    - Max iterations (50): increase to 100-200 for higher precision at cost of speed.
    - Epsilon (1e-5): decrease (e.g. 1e-7) for more precise alignment.
    """
...
def get_indent_intensity(img, cx, cy, radius=5):
    """
    Measures mean pixel intensity inside a circular region around (cx, cy).
    This is the core measurement — intensity changes reflect indent contrast changes.
    
    ⬇ TUNE IF NEEDED:
    - radius=5: sampling circle radius in pixels.
      Increase (e.g. 10-15) for larger indents, decrease (e.g. 3) for very small ones.
      Should be roughly half the visible indent diameter.
    """
...
if __name__ == "__main__":
    ...
    # Key: Indent ID (1-based), Value: {'times': [], 'depths': []}
    global_indent_data = {}
    ...
    # All other images are normalized to match this reference.
    ...
    for seg_idx, (prefix, start_hour) in enumerate(SEGMENT_CONFIG):
        ...
        # Load and normalize the first image of this session as alignment reference
        ...
        # A window will open — click on each indent, then press C to confirm.
        # You can select as many or as few indents as you like.
        ...
        for i, (tx, ty) in enumerate(targets):
            t_template = get_tracking_template(img_ref, tx, ty, size=40)  # ⬅ TUNE: size=40 is patch size
        ...
        for i, fname in enumerate(batch_files):
            ...
            # Align to session reference (skip for first frame)
            if i == 0:
                aligned = img
            else:
                aligned = align_image_ecc(img, img_ref)
                # ⬇ OPTIONAL: If you want to skip alignment entirely, comment out the 3 lines above
            ...
            for indent in active_indents:
                ...
                # If confidence is low, try a wider search window
                if conf <= 0.6:
                    cx, cy, conf = track_template_local(aligned, indent['current_x'], indent['current_y'], indent['template'], search_window=120)
                    if conf <= 0.5:
                        # Fall back to last known position if tracking fails
                        cx, cy = indent['current_x'], indent['current_y']

                # Periodically refresh the tracking template from the current frame
            ...
            # ⬇ OPTIONAL: Change 25 to a different interval, or comment out this block
        ...
        # Removes intensity jump at the boundary between sessions.
        # ⬇ OPTIONAL: Set STITCH_DATA = False at top of script to disable this.
        if STITCH_DATA:
            for indent in active_indents:
                gid = indent['id']
                ...
                if g_depths and s_depths:
                    ...
                    print(f"\n  > Indent {gid}: Applied stitching offset: {offset:.2f}")
    ...
    if any(len(d['times']) > 10 for d in global_indent_data.values()):
        ...
        # ===== COMBINED PLOT — all indents on one chart =====
        ...
        plt.tight_layout()
        plt.savefig("clustering_result_combined.png", dpi=150, bbox_inches='tight')
        plt.show()
        # ===== END COMBINED PLOT =====
        ...
        cols = 5  # ⬅ CHANGE: number of columns in the subplot grid (e.g. 3 for fewer indents, 6 for many)
        ...
        plt.tight_layout()
        plt.savefig("clustering_result_individual.png", dpi=150, bbox_inches='tight')
        plt.show()
        # ===== END INDIVIDUAL PLOTS =====

        # ===== VERIFICATION GRID — tracking accuracy visual check =====
        ...
        if verification_images:
            ...
            cols = 4  # ⬅ CHANGE: columns in verification grid
